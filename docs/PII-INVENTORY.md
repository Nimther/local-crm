...
| `dead_letter_jobs` | A platform-operations record of a terminally-failed BullMQ job (migration `0054`), carrying a `scrub()`-redacted snapshot of the job's payload (`@mega-crm/redaction`'s `scrub`, applied by `writeDeadLetterOnTerminalFailure` before the row is ever written). `scrub()` matches a fixed key-name list plus email/phone/SendGrid-key value patterns, and therefore does **not** cover `firstName`, `lastName`, `externalId`, `testTo`, or a freeform `properties` object by key name -- the same unenumerable tenant-controlled key space this document already refuses to allowlist for `events.properties` above. The table carries no `workspace_id` and is deliberately outside every purge table list (`PURGE_TABLE_ORDER`/`PURGE_SECRET_TABLES`/`PURGE_EVIDENCE_TABLES`) by migration `0054`'s own deliberate platform-scoped design. Gap-closure Phase 22 plan 22-12 (PRG-02): its PII lifetime is instead bounded by `DEAD_LETTER_RETENTION_DAYS` (default 30, floor 7), boot-validated in `apps/worker/src/env.ts` to be at most `WORKSPACE_PURGE_RETENTION_DAYS` -- so any row that existed when a workspace was soft-deleted has already expired before that workspace's own purge becomes eligible. See `docs/runbooks/workspace-purge-and-restore.md`'s "Dead-letter rows" section for the operator-facing residual case. |
