...
describe("ensure-partitions (09-03 task 3, DB-01/DB-04)", () => {
  ...
  beforeAll(async () => {
    ...
    // mega_crm_app (this suite's `pool`) holds only SELECT on organization
    // from this point on, so seeding a workspace row needs the
    // mega_crm_auth-backed DSN for this SAME ephemeral database instead.
