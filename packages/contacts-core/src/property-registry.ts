 ...
 * centralized write path, called from `upsertContactByIdentity` (this
 * package, shared by the Contacts API route, events:ingest worker, and
 * imports:csv worker) and re-exported by `apps/api/src/modules/contacts/
 * property-registry.ts` for the session-authed CRUD path -- four
 * independent "guess the type" implementations would disagree with each
 * other (02-RESEARCH.md Don't Hand-Roll).
 */
...
/** Loops `registerObservedProperty` over every surviving (already reserved-key-stripped) property. */
export async function registerObservedProperties(
  client: PoolClient,
  workspaceId: string,
