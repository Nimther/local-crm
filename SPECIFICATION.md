...
- **Discovery (кросс-тенантный скан):** `findReconcilableCandidates()` — через `withCrossWorkspaceScan` (§5.7, роль `mega_crm_scan`), `SELECT id, workspace_id FROM sends WHERE status = 'reconciling' ORDER BY queued_at LIMIT 500` (`RECONCILER_BATCH_LIMIT = 500`). SELECT-лист состоит РОВНО из этих двух колонок — `mega_crm_scan` не получил грант на `send_events` (миграция `0042`), поэтому классификация по evidence обязана жить в тенант-скоуп-транзакции, не в скане. Без `FOR UPDATE` — блокировка происходит на следующем шаге.
- **Per-tenant claim + классификация:** `resolveOneSend(row)` — `withTenant`/`withTenantTransaction`, `SELECT id, status FROM sends WHERE id = $1 AND status = 'reconciling' FOR UPDATE SKIP LOCKED`; ноль строк → уже забрано конкурентным тиком или уже резолвлено, `false`. Иначе — `SELECT 1 FROM send_events WHERE send_id = $1 LIMIT 1`; evidence нет → строка остаётся в `reconciling` без изменений (переход в `unknown` — экспансия плана 11-07, не эта фаза); evidence есть → `resolveReconcilingSend(client, id, { status: "sent" })` (`packages/delivery-core/src/send-ledger.ts`) — ЕДИНСТВЕННАЯ функция в кодовой базе, которой разрешено писать статус в строку, находящуюся в `reconciling`/`unknown` (D-03), guard'ится собственным `WHERE status IN ('reconciling', 'unknown')`. `sent_at` при резолюции back-dated (`COALESCE(sent_at, dispatched_at, reconciling_since, queued_at)`), никогда `now()` реконсилера — иначе `workspace_daily_rollup`, считающийся по `sent_at::date`, тихо сдвинул бы день.
- **Эксклюзивность (DLV-04), обе половины:** `FOR UPDATE SKIP LOCKED` выше закрывает реконсилер-vs-реконсилер; реконсилер-vs-retry-воркер закрывается **не** локом, а четвёртой веткой статуса в `dispatchSendGate`/`claimFlowSend` (§4, `packages/delivery-core/src/send-ledger.ts`) — редоставленный/повторный джоб для строки в `reconciling`/`unknown` возвращает `"skipped"`, никогда не вызывая SendGrid повторно.
...
- **Полный набор verdict'ов и счётчик-backfill (Phase 11, D-04/D-08/D-12, план 11-08):** `resolveReconcilingSend`'s параметр-verdict расширен до `{ kind: "resolve_sent"; providerMessageId? } | { kind: "resolve_unknown" }` — узкий union без третьего члена делает запись `failed` из реконсилера ошибкой компиляции, а не поводом для ревью. `resolve_sent` резолвит `reconciling`/`unknown` → `sent` (back-dated `sent_at = COALESCE(dispatched_at, reconciling_since, queued_at)`, поле `sent_at` убрано из цепочки COALESCE как избыточное — оно гарантированно NULL на входе); `resolve_unknown` резолвит `reconciling` → `unknown`, не трогая `sent_at`/`reconciling_since` вовсе. Новая `sweepStaleDispatchingSend(client, sendId)` — второй писатель перехода `dispatching → reconciling` из матрицы ARCHITECTURE.md §9 (первый — сама ветка `interrupted`/ambiguous воркера): `UPDATE ... WHERE status = 'dispatching'`, guard которого делает проигрыш гонки с воркером безопасным no-op. Новая `backfillCampaignSendCounter(client, campaignId, status)` — `incrementCampaignSendCounter` без гварда `WHERE status = 'sending'`, вызываема ТОЛЬКО из транзакции `resolveReconcilingSend`, и только когда та реально сообщила о переходе (exactly-once следует из эксклюзивности этого перехода под `FOR UPDATE SKIP LOCKED`, не из отдельного флага). `tryCompleteCampaign`'s предикат завершения расширен: `(sent_count + failed_count + count(sends WHERE campaign_id=$1 AND status IN ('reconciling','unknown'))) >= sendable_total` — кампания, чей последний неотправленный получатель неоднозначен, всё равно завершается; `dispatching` сознательно НЕ входит в подзапрос (ещё реально в полёте, не неоднозначен). Все четыре функции добавлены в этом плане на уровне `send-ledger.ts`; их вплетение в `resolveOneSend`/`runReconcilerTick` — та же секция ниже, следующий commit этого же плана.
