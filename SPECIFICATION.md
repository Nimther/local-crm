...
- **Четыре независимых пула** (было три до Phase 10, было два до 09-REVIEW CR-03), на **разных** DSN начиная с Phase 10:
  ...
  4. **Phase 10 (SEC-01/SEC-02):** `packages/tenant-context/src/scan.ts` → `getScanPool()`, лениво создаваемый `new Pool({ connectionString: process.env.SCAN_DATABASE_URL })` — единственный из четырёх постоянных пулов на **отдельном от `DATABASE_URL` DSN**, соединяется под ролью `mega_crm_scan`, не под `mega_crm_app`. Лениво — импорт пакета из `apps/api` ничего не конструирует, поскольку `SCAN_DATABASE_URL` в API-окружении не объявлена (§3.2, P3). Есть `pool.on("error")`-хендлер (тот же паттерн). Потребители: `campaign-scheduler.worker.ts`, `flow-reconciliation.worker.ts`, `flow-segment-sweep.worker.ts`, `analytics-reconciliation.worker.ts`'s `findDueCampaignCandidates`/`findDueFlowRunCandidates`/`findLiveSegmentTriggeredFlows`/организационный enum — все четыре named cross-tenant scan consumer'а фазы, кроме partition maintenance/relocation (см. пул #5 ниже).
  5. **Phase 10, план 10-06 (SEC-01/SEC-02, чекпойнт option-b), CLI-only, не постоянный процесс:** `packages/db/scripts/relocate-default-partition-rows.ts` конструирует ДВА `Pool` за один запуск — один на `DATABASE_URL` (обычная роль `mega_crm_app`), второй на `PARTITION_RELOCATION_ADMIN_DATABASE_URL` (§3.2) под ролью, способной обходить row-level security. Второй передаётся как `adminClient` в `relocateAllDefaultRows`/`relocateMonth`/`attachPartitionCheckFirst` и используется только на шаге `ATTACH PARTITION`. Существует только на время работы оператор-invoked CLI — не пул сервисного процесса, поэтому не входит в счёт «четырёх независимых пулов» выше.
