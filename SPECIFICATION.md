...
### 4.2 Доменные таблицы (22 из 24 — с RLS ENABLE + FORCE; исключения — `partition_maintenance_runs` и `send_reconciler_runs` (`0050`, Phase 11), см. конец раздела)
...
**Политика `workspace_isolation`** есть на каждой из 22 доменных таблиц, `USING` и `WITH CHECK` совпадают, и с миграции `0044_workspace_isolation_fail_closed.sql` (план 10-07, SEC-03/SEC-04) существует в **одном** унифицированном, fail-closed, явно роль-скоупленном варианте:
...
### 5.2 Зарегистрированные воркеры (15)

`events-ingest`, `imports-csv`, `email-broadcast`, `email-triggered`, `campaign-kickoff`, `campaign-scheduler`, `webhook-events`, `analytics-reconciliation`, `flow-run-advance`, `flow-reconciliation`, `flow-trigger-evaluator`, `flow-segment-sweep`, `flow-enroll-existing`, `partition-maintenance` (09-02), `send-reconciler` (11-03, пятнадцатый в композиционном корне `buildWorker()`, см. §5.10).
...
`events-ingest`, `imports-csv`, `email-broadcast`, `email-triggered`, `campaign-kickoff`, `webhook-events`, `flow-trigger-evaluator`, `flow-run-advance`, `flow-reconciliation`, `flow-segment-sweep`, `flow-enroll-existing`, `send-reconciler` (11-03, `SEND_RECONCILER_QUEUE`). Плюс два тик-имени, объявленных локально в файлах воркеров: `campaign-scheduler`, `analytics-reconcile`. Плюс `partition-maintenance` (09-02, объявлена локально в `partition-maintenance.worker.ts` как `PARTITION_MAINTENANCE_QUEUE`) — **самопродюсируемая и самопотребляемая** очередь: единственный producer этой очереди — сам процесс `apps/worker`, тот же файл, что её и consume'ит; никакой роут или другой воркер в неё не пишет. Дефис вместо двоеточия — BullMQ запрещает `:` в имени очереди. `send-reconciler` — та же самопродюсируемая/самопотребляемая форма, что и `partition-maintenance` (см. §5.10).
