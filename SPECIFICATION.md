...
| `app.admin_scan` | `ensure-partitions.ts` → `attachPartitionCheckFirst` (внутри своей транзакции, сразу после `BEGIN`) | `partition_relocation_admin_scan` на `contacts` **и** на `sends` (`0039`) | **`current_setting('app.admin_scan', true) = 'true'` — и всё** |

⚠️ Последние три политики, вопреки собственным комментариям («mirror the 0018 precedent exactly»), **не содержат никакого предиката помимо проверки GUC**. Установка `app.admin_scan='true'` открывает на чтение **все строки `flows`/`flow_runs`/`contacts`/`sends` во всех воркспейсах, в любом статусе**. Фактические SQL-запросы воркеров/DDL сужают выборку сами (`WHERE status='waiting' AND next_wake_at<=now()` и т.п.), но политика не ограничивает ничего.
...
- Журнал: `migrations/meta/_journal.json`, version 7, 41 запись (0–40, `0040` добавлена 09-REVIEW CR-01 — сид singleton-строки `partition_maintenance_runs`). Snapshot'ы есть только у **11** миграций (`0000`, `0002`, `0003`, `0005`, `0008`, `0011`, `0016`, `0017`, `0024`, `0025`, `0034`); у остальных **30** (включая `0038`, `0039`, `0040`) — нет.
