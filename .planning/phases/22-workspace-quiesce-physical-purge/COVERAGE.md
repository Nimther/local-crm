...
|---|---|---|
| `DELETE /v3/user/webhooks/event/settings/{id}` — deprovision the tenant's event webhook before key destruction | OPT-OUT | Out of CONTEXT.md scope ("no re-planning of existing subsystems"); the orphaned subscription lives in the tenant's own account, 404s harmlessly against the tombstoned pathToken, and the tenant can remove it. Documented in the purge runbook. |
