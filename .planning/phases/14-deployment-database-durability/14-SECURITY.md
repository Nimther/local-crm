...
| T-14-49 | Tampering | deploying an unreviewed local tree | high | mitigate | Application-image path pull-only on immutable SHA tags, gate-asserted (UAT #7). Evidence narrowed 2026-08-14: plan 14-10 added `build:` sections for the host-built `db`/`pgbackrest` image (no COPY of repository source — infra config only); residual surface tracked as T-14-88. **Phase 17 (2026-08-19):** the narrowed residual surface this row hands off to is no longer merely tracked — see T-14-88's Phase 17 evidence clause above (immutable-tag gate now covers `db`/`pgbackrest`, `build:` sections removed, image is CI-built/GHCR-published). This row's own status remains `closed`, unaffected by Phase 17. | closed |
...
| T-14-58 | EoP | host-built image bypassing review | medium | mitigate | Partially present: deploy.sh never builds and app images stay pull-only under the immutable-tag gate, but 14-10 added `build:` sections for `db`/`pgbackrest` (mutable `POSTGRES_IMAGE_TAG`, excluded from FIRST_PARTY_IMAGE_SERVICES) — documented forward flag (backups.md:270-280); tracked with T-14-88. **Phase 17 evidence (2026-08-19, pending auditor re-run — see note below):** `build:` sections removed from `docker/docker-compose.prod.yml`'s `db`/`pgbackrest` (plan 17-03); image reference is now `${GHCR_IMAGE_BASE}/postgres:${POSTGRES_IMAGE_TAG}` with no fallback; built and pushed by `.github/workflows/images.yml`'s `build-and-push-postgres` job on the same `github.sha` as the application images. Live cutover observed: `docker inspect`'s `Config.Image` reported the GHCR reference and the running image id changed (`sha256:de6a69e4...` → `sha256:e718495c...`) — see `.planning/phases/17-address-tech-debt-wr-06-medium-security-follow-ups/17-05-SUMMARY.md`'s "Task 1 — Attempt 3 (APPROVED...)" section. **Closed 2026-08-20 by gsd-security-auditor re-run (`/gsd-secure-phase 17`, D-12): both halves of the evidence clause verified independently — zero `build:` keys in `docker/docker-compose.prod.yml`, `db`/`pgbackrest` pull `${GHCR_IMAGE_BASE}/postgres:${POSTGRES_IMAGE_TAG}` published by `images.yml`'s `build-and-push-postgres` on the same `github.sha` as the app images; live cutover pasted in 17-05-SUMMARY.md (Attempt 3, no rollback). The review-bypass path no longer exists.** | closed |
...
| T-14-73 | DoS | drill exhausting host disk | medium | mitigate | Free-disk prerequisite (`df -h`) + teardown reclaim present; observed figure still runbook placeholder — capture at next scheduled drill (tracked in 14-11 SUMMARY and STATE.md). **Phase 17 evidence (2026-08-19, pending auditor re-run — see note below):** `scripts/restore-drill.sh` now self-records restore-to-ready duration and scratch-PGDATA disk high-water on every run, including the readiness-timeout path, to stdout and to an append-only `RESTORE_DRILL_METRICS_FILE` history file (plan 17-04). Real figures captured from a real PITR drill against the CI-built image on the production host: `durationSeconds=119`, `diskHighWaterKb=170520` — see `17-05-SUMMARY.md`'s "Task 2 — restore drill (APPROVED)" section for the verbatim metrics NDJSON line, and `docs/runbooks/restore-drill.md`'s "Recorded drill runs" table where the figures and the derived free-disk-headroom requirement live (the derived headroom requirement, not merely the measurement, is what makes this a mitigation). **Closed 2026-08-20 by gsd-security-auditor re-run (`/gsd-secure-phase 17`, D-12): recording is structural, not operator memory — `restore-drill.sh` appends NDJSON on both the success and readiness-timeout paths (47/47 tests green); real figures match digit-for-digit between the SUMMARY NDJSON (`durationSeconds:119, diskHighWaterKb:170520`) and `docs/runbooks/restore-drill.md`'s recorded-runs table; the derived operational rule (free disk ≥ 2× current PGDATA size) exists at `restore-drill.md:287-300`.** | closed |
...
| T-14-88 | Tampering | host-built `db`/`pgbackrest` image (mutable `POSTGRES_IMAGE_TAG`, outside immutability gate) | medium | mitigate | Registered 2026-08-14 (successor to T-14-49's narrowed evidence). No COPY of repository source (infra config only) keeps this below high. Recommended: extend FIRST_PARTY_IMAGE_SERVICES or add a distinct `POSTGRES_IMAGE_TAG` immutability check. **Phase 17 evidence (2026-08-19, pending auditor re-run — see note below):** `db` and `pgbackrest` added to `scripts/validate-prod-compose.mjs`'s `FIRST_PARTY_IMAGE_SERVICES`; `"local"` added to `MUTABLE_TAG_NAMES` (plan 17-03). Fixture `scripts/__fixtures__/prod-compose/db-mutable-image-tag.yml` plus its case in `scripts/__tests__/validate-prod-compose.test.mjs` prove the gate rejects a mutable tag on `db` (run via `npx vitest run scripts/__tests__/validate-prod-compose.test.mjs`, 34/34 pass per plan 17-03's SUMMARY). `docker/prod.env.example`'s default is now the deliberately-invalid 40-zero placeholder (was `local`). `scripts/restore-drill.sh` gained fail-loud `:?`-guarded `GHCR_IMAGE_BASE`/`POSTGRES_IMAGE_TAG` checks (plan 17-04) so a drill can no longer silently fall back to a stale local image. **Closed 2026-08-20 by gsd-security-auditor re-run (`/gsd-secure-phase 17`, D-12): the gate was confirmed to work, not merely exist — the `db-mutable-image-tag.yml` fixture case executed 34/34 pass; the old `local` default is eliminated (40-zero placeholder); the `:?` guards were observed firing; production runs the SHA-tagged GHCR image live.** | closed |
...
| 2026-08-19/20 | same 97 (no new threats registered) | same 94 (no status change) | same 3 medium, all below `high` block threshold — threats_open: 0 | Phase 17, plan 17-06 (executor documentation pass, NOT a security audit — see note below) |
| 2026-08-20 | same 97 | 97 (T-14-58, T-14-73, T-14-88 flipped to closed — full register now closed) | 0 — threats_open: 0 | gsd-security-auditor re-run via `/gsd-secure-phase 17` (D-12 fulfilled): each row's Phase 17 evidence clause verified against the implementation files and 17-05-SUMMARY.md's pasted live output; verdict FLIP-TO-CLOSED ×3 — see the dated closure clause on each row |

**Phase 17 supplied closure evidence for T-14-58/T-14-73/T-14-88 (2026-08-19/20); the three rows' status is deliberately UNCHANGED, pending a `gsd-security-auditor` re-run.** Plans 17-03/17-04 closed the implementation gaps this register's three open medium rows described (CI-built/GHCR-published `db`/`pgbackrest` image with an immutable-tag gate; self-recorded restore-drill metrics with a derived disk-headroom rule); plan 17-05 then produced live production evidence (a real cutover to the CI-built image, and a real PITR restore drill against the real off-host repository) — see each row's "Phase 17 evidence" clause above and `.planning/phases/17-address-tech-debt-wr-06-medium-security-follow-ups/17-05-SUMMARY.md`. **An executor is not permitted to award these verdicts** (D-12, `.planning/phases/17-address-tech-debt-wr-06-medium-security-follow-ups/17-CONTEXT.md`) — the register reached `verified` originally through a `gsd-security-auditor` pass, and rows flip to `closed` the same way, never by executor edit. **Next action: run `/gsd-secure-phase` against this register** to re-verify T-14-58/T-14-73/T-14-88 against the Phase 17 evidence cited above and award (or withhold) the status flip.

**Fulfilled 2026-08-20.** The required `gsd-security-auditor` re-run executed under `/gsd-secure-phase 17` and awarded FLIP-TO-CLOSED on all three rows — each evidence clause was independently re-verified against the implementation files (gates executed, tests run: `validate-prod-compose` 34/34, `restore-drill`+`deploy-script` 47/47) and against `17-05-SUMMARY.md`'s pasted live production output. The three rows above now read `closed` with a dated closure clause each; the paragraph above is retained as the decision trail for why the flip waited. Full verdict: `.planning/phases/17-address-tech-debt-wr-06-medium-security-follow-ups/17-SECURITY.md`.
...
- [x] `threats_open: 0` confirmed (full register closed as of 2026-08-20 — the three medium follow-ups T-14-58/T-14-73/T-14-88 were flipped to closed by the gsd-security-auditor re-run under `/gsd-secure-phase 17`)
- [x] `status: verified` set in frontmatter

**Approval:** verified 2026-08-14 — deferred register for plans 14-09/14-10/14-11 audited post-execution; the 2026-08-13 conditional re-run instruction is fulfilled. Open medium follow-ups: T-14-58/T-14-88 (host-built db/pgbackrest image immutability — candidate for a future hardening pass), T-14-73 (record drill duration + disk high-water at the next scheduled restore drill). **Update (2026-08-19/20):** Phase 17 supplied closure evidence for all three follow-ups — see the "Phase 17 evidence" clauses on each row and the dated note in the Security Audit Trail section above. Status intentionally unchanged pending `/gsd-secure-phase` (D-12). **Update (2026-08-20):** the pending re-run executed (`/gsd-secure-phase 17`, gsd-security-auditor) and closed all three rows — the register is now fully closed; see the audit trail row and per-row closure clauses above.
