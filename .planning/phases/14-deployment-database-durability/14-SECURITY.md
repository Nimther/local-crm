...
| T-14-88 | Tampering | host-built `db`/`pgbackrest` image (mutable `POSTGRES_IMAGE_TAG`, outside immutability gate) | medium | mitigate | Registered 2026-08-14 (successor to T-14-49's narrowed evidence). No COPY of repository source (infra config only) keeps this below high. Recommended: extend FIRST_PARTY_IMAGE_SERVICES or add a distinct `POSTGRES_IMAGE_TAG` immutability check. **Phase 17 evidence (2026-08-19, pending auditor re-run — see note below):** `db` and `pgbackrest` added to `scripts/validate-prod-compose.mjs`'s `FIRST_PARTY_IMAGE_SERVICES`; `"local"` added to `MUTABLE_TAG_NAMES` (plan 17-03). Fixture `scripts/__fixtures__/prod-compose/db-mutable-image-tag.yml` plus its case in `scripts/__tests__/validate-prod-compose.test.mjs` prove the gate rejects a mutable tag on `db` (run via `npx vitest run scripts/__tests__/validate-prod-compose.test.mjs`, 34/34 pass per plan 17-03's SUMMARY). `docker/prod.env.example`'s default is now the deliberately-invalid 40-zero placeholder (was `local`). `scripts/restore-drill.sh` gained fail-loud `:?`-guarded `GHCR_IMAGE_BASE`/`POSTGRES_IMAGE_TAG` checks (plan 17-04) so a drill can no longer silently fall back to a stale local image. | open (below block threshold) |
...
**Approval:** verified 2026-08-14 — deferred register for plans 14-09/14-10/14-11 audited post-execution; the 2026-08-13 conditional re-run instruction is fulfilled. Open medium follow-ups: T-14-58/T-14-88 (host-built db/pgbackrest image immutability — candidate for a future hardening pass), T-14-73 (record drill duration + disk high-water at the next scheduled restore drill). **Update (2026-08-19/20):** Phase 17 supplied closure evidence for all three follow-ups — see the "Phase 17 evidence" clauses on each row and the dated note in the Security Audit Trail section above. Status intentionally unchanged pending `/gsd-secure-phase` (D-12).
