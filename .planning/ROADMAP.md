...
- [ ] **Phase 12: Worker Reliability & Tenant Fairness** - One tenant, one huge segment, or a restart cannot degrade the platform
...
| **Per-tenant concurrency-cap mechanism (WRK-02)** | Phase 12 | BullMQ configuration **vs.** a Redis counter **vs.** bounded per-tier worker pools. Distinct from RPS throttling — a correct token bucket still lets one tenant's backlog occupy slots ahead of another tenant's small batch. |
...
| 8. Quality Gates & Failure-Injection Foundation | v1.1 | 13/18 | In Progress|  |
...
| 12. Worker Reliability & Tenant Fairness | v1.1 | 0/TBD | Not started | - |
