 ...
 *
 * Redundant safety net: env.ts's schema-level `superRefine` is the primary
 * "refuse to boot" guard (fires before the server even starts listening,
 * regardless of whether this module is ever imported). This module-level
 * check additionally guards the case where something imports this module
 * directly while NODE_ENV=production, independent of KMS_PROVIDER.
 */
if (env.NODE_ENV === "production") {
  throw new Error(
    "kms/local-provider.ts must never be imported when NODE_ENV=production (RESEARCH.md Pitfall 3) -- set KMS_PROVIDER=aws and KMS_KEK_ID instead."
  );
}
