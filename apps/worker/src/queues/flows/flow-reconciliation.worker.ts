 ...
 * T-06-05-02): `pool.connect()` directly (NOT `withTenant`/
 * `withTenantTransaction` -- this scan doesn't know which workspace a run
 * belongs to until it reads one) inside a short, READ-ONLY transaction that
 * sets ONLY the narrow `app.admin_scan` GUC (migration 0027's
 * `flow_runs_due_scan` policy, a SELECT-only permissive policy covering
 * `flow_runs` alone -- NOT `flows`, so this candidate list may include runs
 * belonging to a `paused` flow; the per-tenant re-verification below is
 * what actually filters those out, D-18/D-19). Deliberately NOT `FOR
 * UPDATE` here, mirroring `campaign-scheduler.worker.ts`'s
 * `findDueCampaignCandidates` exactly: the row-level lock for the actual
 * transition happens per-tenant in `transitionAndNudge`, below.
 */
