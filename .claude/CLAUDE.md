...
- новый npm-пакет (runtime или dev) → **раздел 2 «Зависимости и версии»**, в таблицу нужного workspace, с точной версией из `package.json`
- новая переменная окружения, секрет, провайдер KMS или способ загрузки конфига → **раздел 3 «Секреты»**
- новая таблица, колонка, миграция, RLS-политика, индекс или партиция → **раздел 4 «Схема данных»**
- новая очередь, воркер, repeatable job или изменение интервала/concurrency → **раздел 5 «Планировщик и пайплайн отправки»**
- новый HTTP-роут, плагин Fastify, парсер тела, механизм аутентификации или rate limit → **раздел 6 «Публичные точки входа»**
- логирование, метрики, трейсинг, healthcheck → **раздел 7 «Наблюдаемость»**
- если технология расходится с разделом Technology Stack выше (нет там / другая версия / заменена) → дополнительно **раздел 8 «Расхождения»**
